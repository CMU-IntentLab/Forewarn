...
def sample_to_device(
    batch_size, success_dataset, device, remove_obs_stack=False
):
...
def train_eval(config):
    ...
    with open(f"{logdir}/config_xmagical.yaml", "w") as f:
    ...
    observation_space, action_space = tools.fill_expert_dataset_xmagical(config, success_eps, dataset_type='success')
    ...
    #
    # tools.fill_expert_dataset_xmagical(config, failure_eps, dataset_type='failure')
    # failure_dataset = make_dataset(failure_eps, config)
    
    
    # validation replay buffer
    success_val_eps = collections.OrderedDict()
    tools.fill_expert_dataset_xmagical(config, success_val_eps, is_val_set=True, dataset_type='success')
    success_val_dataset = make_dataset(success_val_eps, config)
    # failure_val_eps = collections.OrderedDict()
    # tools.fill_expert_dataset_xmagical(config, failure_val_eps, is_val_set=True, dataset_type='failure')
    ...
    env_name = 'xmagical-longstick-img'
    ...
    if config.from_ckpt and Path(config.from_ckpt).exists():
        ...
        pretrained_ckpt_config = Path(config.from_ckpt).parent / "config_xmagical.yaml"
    ...
    def eval_obs_recon():
        ...
        return np.min(eval_loss_success)
        ...
        for step in trange(
            ...
            if (
                ...
                score, success = evaluate(
                    other_dataset=[success_dataset], eval_prefix="pretrain"
                )
            ...
            sample = sample_to_device(success_dataset=success_dataset, batch_size=config.batch_size, device=config.device)
...
if __name__ == "__main__":
    ...
    configs = yaml.load(
        (pathlib.Path(sys.argv[0]).parent / "../configs/config_xmagical.yaml").read_text()
    )
    ...
    final_config.time_limit = HORIZONS[final_config.task]
